...
/**
 * 操作记录工具对象
 * 通过 KoinComponent 注入依赖
 */
object OperationUtil : KoinComponent {
    private val operationService: OperationService by inject()
...
suspend fun operateSync(
    ...
    username: String,
    isHighRisk: Boolean = false
) = OperationUtil.operateSync(desc, userId, username, isHighRisk)
...
fun operate(
    ...
    username: String,
    isHighRisk: Boolean = false
) = OperationUtil.operate(desc, userId, username, isHighRisk)
...
fun operate(
    ...
    call: ApplicationCall,
    isHighRisk: Boolean = false
) = OperationUtil.operate(desc, call, isHighRisk)
